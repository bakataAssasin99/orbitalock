    ...
    begin
     ...
     // int ValidTimes);
       ...
       if( Ret =0 ) then
         ...
         MessageBox(0,pchar('Issue card failed£¡Error Code£º'+IntToStr(Ret)),'Failed£¡',0+48);
    end
    ...
    if assigned(F_TReadMessage) then
      begin
        ...
        Ret:= F_TReadMessage(
            ...
            if Ret =0  then
              begin
                MessageBox(0,pchar('Read card successfully£¡CardNo£º'+IntToStr(CardNumber) ),'Read Card Success',MB_OK)
              end
            else
